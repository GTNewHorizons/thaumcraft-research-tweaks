...
class Runes(
    ...
    fun forEach(action: (Rune) -> Unit) =
        keysToRunes.values.forEach(action)
    ...
    private fun obstructingExisting(aspectHex: AspectHex?) =
        (research.incomplete() && aspectHex != null) || (!research.incomplete() && aspectHex is AspectHex.Occupied)

    private fun decayRunes(partialTicks: Float) {
        val decayedRunes = keysToRunes
            .onEach { (_, rune) -> rune.decay(partialTicks) }
